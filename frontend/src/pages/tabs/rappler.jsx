...
const RAPPLER = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        setLoading(true);
        const response = await axios.get('http://202.175.224.155:8080/api/rp/get-data');
  ...
  const navigateArticle = async (url) => {
    ...
    try {
      let response = await axios.post('http://202.175.224.155:8080/api/rp/get-page', { url });
      ...
      try {
        const response = await axios.post('http://202.175.224.155:8080/api/rp/page', { url });
