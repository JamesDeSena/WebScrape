...
const RAPPLER = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${API}/api/rp/get-data`);
  ...
  const navigateArticle = async (url) => {
    ...
    try {
      let response = await axios.post(`${API}/api/rp/get-page`, { url });
      ...
      try {
        const response = await axios.post(`${API}/api/rp/page`, { url });
