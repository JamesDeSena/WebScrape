...
const MBulletin = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        setLoading(true);
        const response = await axios.get('https://webscrape-5iyk.onrender.com/api/mb/get-data');
  ...
  const navigateArticle = async (url) => {
    ...
    try {
      let response = await axios.post('https://webscrape-5iyk.onrender.com/api/mb/get-page', { url });
      ...
      try {
        const response = await axios.post('https://webscrape-5iyk.onrender.com/api/mb/page', { url });
