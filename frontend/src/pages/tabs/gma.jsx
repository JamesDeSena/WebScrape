...
const GMA = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        setLoading(true);
        const response = await axios.get('http://localhost:8008/api/gma/get-data');
  ...
  const navigateArticle = async (url) => {
    ...
    try {
      const response = await axios.post('http://localhost:8008/api/gma/get-page', { url });
      ...
      try {
        const response = await axios.post('http://localhost:8008/api/gma/page', { url });
