...
const PHStar = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${API}/api/ps/get-data`);
  ...
  const navigateArticle = async (url) => {
    ...
    try {
      let response = await axios.post(`${API}/api/ps/get-page`, { url });
      ...
      try {
        const response = await axios.post(`${API}/api/ps/page`, { url });
