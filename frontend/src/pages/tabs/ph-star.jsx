...
const PHStar = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        setLoading(true);
        const response = await axios.get('http://localhost:8008/api/ps/get-data');
  ...
  const navigateArticle = async (url) => {
    ...
    try {
      let response = await axios.post('http://localhost:8008/api/ps/get-page', { url });
      ...
      try {
        const response = await axios.post('http://localhost:8008/api/ps/page', { url });
