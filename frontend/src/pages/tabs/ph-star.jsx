...
const PHStar = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        setLoading(true);
        const response = await axios.get('https://pdmnnewshub.ddns.net:8080/api/ps/get-data');
  ...
  const navigateArticle = async (url) => {
    ...
    try {
      let response = await axios.post('https://pdmnnewshub.ddns.net:8080/api/ps/get-page', { url });
      ...
      try {
        const response = await axios.post('https://pdmnnewshub.ddns.net:8080/api/ps/page', { url });
