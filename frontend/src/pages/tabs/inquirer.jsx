...
const Inquirer = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        setLoading(true);
        const response = await axios.get('http://localhost:8008/api/inq/get-data');
  ...
  const navigateArticle = async (url) => {
    ...
    try {
      let response = await axios.post('http://localhost:8008/api/inq/get-page', { url });
      ...
      try {
        const response = await axios.post('http://localhost:8008/api/inq/page', { url });
