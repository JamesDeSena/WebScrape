...
const BWorld = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        setLoading(true);
        const response = await axios.get('http://192.168.13.206:8008/api/bw/get-data');
  ...
  const navigateArticle = async (url) => {
    ...
    try {
      let response = await axios.post('http://192.168.13.206:8008/api/bw/get-page', { url });
      ...
      try {
        const response = await axios.post('http://192.168.13.206:8008/api/bw/page', { url });
